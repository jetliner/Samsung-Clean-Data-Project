...
- An explicit and exact recipe you used to go from raw data to tidy data.
- Information on the raw data and the study design

## Tidy Data set variables

- ACTIVITY_DESC : string describing the activity (per activity_labels.txt file)
- SUBJECT_ID: integer identifing one of the study participants.
- t*: set of variables representing the averages of the raw input variables by activity, by subject for all inpt variables that represent "mean" or "standard deviation"

## Transformation steps to get from  raw data to tidy data:

- 1. Merges the training and the test sets to create one data set.
- 2. Extracts only the measurements on the mean and standard deviation for each measurement.  Any variable with mean() or std() in its name is included.
...
- 5. From the data set above,uses the aggregrate() function to create a second, independent tidy data set with the average of each variable for each activity and each subject.
- 6. write the data set out to the output file in the working directory


## Preconditions 

- run_analysis.R is stored in the working directory 
- the working directory contains both the file and the uncompressed Samsung data files.
...
## Raw Data and Study Design Info as provided by SmartLab
